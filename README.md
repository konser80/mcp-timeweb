...
Every DNS tool takes `domain` (zone FQDN) and an optional `subdomain` label. For non-TXT record types, the subdomain entity is auto-created in the Timeweb panel on first use (409 "already exists" is silently tolerated). TXT records set the subdomain inline — no panel entity is created. `timeweb_delete_subdomain` removes the subdomain entity and any DNS records under it.
...
- No domain registration, renewal, or mailbox management.
- No proxy support.
